...
| 流计算运行失败             | 每 60 秒检查一次，当某个流在最近 120 秒内有失败事件上报（taosd 上报至 `log.taosd_stream_failure`），会触发告警，告警信息包含流名称、集群 ID 和错误码 |
| 流计算重算失败             | 每 60 秒检查一次，当存在状态为 Failed 的流计算重算任务，会触发告警，告警信息包含流名称、重算任务 ID、失败时的进度和失败原因（`message` 列）       |

**注意**：流计算运行失败、流计算重算失败两条规则依赖流计算可观测性功能，仅 TDengine **3.4.2.7** 及以上版本支持。在更早的版本上这两条规则会处于查询错误状态，不影响其他告警规则。加上这两条后，自动导入的告警规则共 16 条。
